...
async function makeRequest(task, mode, testName) {
    return new Promise((resolve, reject) => {
        const startTime = Date.now();
        const data = JSON.stringify({ task, mode });
        ...
        console.log(`${colors.magenta}Mode: ${mode || 'balanced (default)'}${colors.reset}`);
            ...
            res.on('end', () => {
                ...
                try {
                    ...
                    if (parsed.success && parsed.data) {
                        ...
                        if (parsed.usage) {
                            console.log(`${colors.yellow}Cost: $${parsed.usage.estimatedCostUSD.toFixed(6)}${colors.reset}`);
                            console.log(`${colors.yellow}Total Tokens: ${parsed.usage.totalTokens}${colors.reset}`);
                        }
...
async function runTests() {
    ...
    console.log(`${colors.bright}  Phase 2.5 Mode-Based Routing Tests${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}============================================${colors.reset}`);

    const task = 'Explain quantum computing in one sentence';

    const tests = [
        { name: 'Default Mode (Balanced)', task, mode: undefined },
        { name: 'Fast Mode (Gemini First)', task, mode: 'fast' },
        { name: 'Quality Mode (Anthropic First)', task, mode: 'quality' },
        { name: 'Balanced Mode (OpenAI First)', task, mode: 'balanced' },
    ];

    for (const test of tests) {
        try {
            await makeRequest(test.task, test.mode, test.name);
