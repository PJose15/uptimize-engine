...
export default function PortalOverview() {
    ...
    const hoursSavedTrend = analytics?.hours_saved_trend || [];
        ...
        <div className="p-8 max-w-6xl">
            ...
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">
                    {clientLoading ? (
                        <span className="inline-block h-7 w-48 bg-zinc-200 dark:bg-zinc-700 rounded animate-pulse" />
                    ) : (
                ...
                {!clientLoading && client && (
                    <p className="text-zinc-500 dark:text-zinc-400 mt-1">
                        Here&apos;s what <span className="font-medium text-violet-600 dark:text-violet-400">{client.agent.name}</span> has been doing for {client.company}.
                    </p>
                )}
            </div>

            {statsError && <div className="mb-6"><ErrorBanner message={statsError} onRetry={refetchStats} /></div>}
